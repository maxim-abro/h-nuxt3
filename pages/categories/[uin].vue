<template>
  <div class="">
    ...
    <h1 class="text-2xl lg:text-3xl font-bold mb-4">
      ...
      />Промокоды на скидку в категории {{ category.title.toLowerCase() }}
    </h1>
...
const responseCategory = await $fetch<CategoryType>(
  `https://za-halyavoi.ru/api/category/by/${route.params.uin}`,
);
const responseShops = await $fetch<ShopType[]>(
  `https://za-halyavoi.ru/api/shop/category/${responseCategory.uin}`,
);
