...
const page: Ref<UnwrapRef<string | number | LocationQueryValue[]>> = ref(route.query.page || 1)

const responseShop = await $fetch<ShopType>(`https://za-halyavoi.ru/api/shop/${route.params.uin}`)
const responsePosts = await $fetch<{count: number, rows: PostType[]}>(`https://za-halyavoi.ru/api/post?shop=${responseShop.uin}&page=${page.value}`)
const responseRecommended = await $fetch<PostType[]>('https://za-halyavoi.ru/api/post/recommended', {
  method: 'post',
  body: {
